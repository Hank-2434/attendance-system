...
# ------------------ DATABASE PATH ------------------
# Automatically choose path depending on environment
if os.environ.get("RENDER") == "true":
    DB_NAME = "/tmp/attendance.db"  # Render writable path
else:
    DB_NAME = os.path.join(os.path.dirname(__file__), "attendance.db")  # Local
...
# ------------------ TEMPORARY: Assign Adviser Roles ------------------
@app.route("/assign_advisers")
def assign_advisers():
    # Only allow yourself to run this
    if session.get("username") != "Hank":  # replace with YOUR login
        return "Access denied"

    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    advisers = ["Hank", "friend_username", "Rey Batiles"]  # replace with actual usernames
    cursor.execute(
